...
impl AdminService {
    ...
    pub async fn add_credential(
        ...
        let new_cred = KiroCredentials {
            ...
            issuer_url: req.issuer_url.filter(|s| !s.trim().is_empty()),
            scopes: req.scopes.filter(|s| !s.trim().is_empty()),
    ...
    pub fn set_load_balancing_mode(
        ...
        // 验证模式值
        if req.mode != "priority" && req.mode != "balanced" && req.mode != "auto" {
            return Err(AdminServiceError::InvalidCredential(
                "mode 必须是 'priority'、'balanced' 或 'auto'".to_string(),
            ));
        }
